...
모아북 안드로이드 앱에 "내 서점"(읽고싶은 책 목록, `ShelfType.STORE`)을 노출하는 홈화면 위젯을 추가한다. 사이즈는 **S(110×110, 2×2 셀), M(250×110, 4×2 셀), L(250×250, 4×4 셀)** 세 가지 — Android Widget 셀 산정 공식 `70 × n − 30` 기준. 모든 위젯이 같은 데이터 소스(`MyBookRepository.getStoreBooks(...)`)를 사용한다.
...
> **사이즈 산정 공식**: Android 11 이하 + 일부 Android 12+ 런처는 `cells = round((dp + 30) / 70)` 으로 셀 수를 결정한다. 따라서 `minWidth/minHeight` 는 `70 × n − 30` (2셀=110dp, 4셀=250dp) 으로 산정한다. `targetCellWidth/Height` 는 API 31+ 에서만 우선 동작하므로 minWidth/Height 도 정확한 셀 수에 맞춰야 모든 기기에서 의도한 그리드를 차지한다.
>
> Figma 디자인의 픽셀 사이즈(158/338/354)는 이 공식과 어긋나 일부 런처에서 2×2 가 아닌 2×3 ~ 3×3 으로 보이는 문제가 있어 dp 값은 디자인 픽셀이 아닌 위 공식을 따른다. 콘텐츠 레이아웃은 위젯이 차지한 실제 셀 크기에 fillMaxSize 로 적응한다.

### 3.1 S (Small) — 110dp × 110dp (2×2 셀)
...
### 3.2 M (Medium) — 250dp × 110dp (4×2 셀)
