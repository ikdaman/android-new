package project.side.domain.usecase.mybook

import io.mockk.MockKAnnotations
import io.mockk.coEvery
import io.mockk.coVerify
import io.mockk.impl.annotations.MockK
import kotlinx.coroutines.flow.flowOf
import kotlinx.coroutines.flow.toList
import kotlinx.coroutines.test.runTest
import org.junit.Assert.assertEquals
import org.junit.Before
import org.junit.Test
import project.side.domain.DataResource
import project.side.domain.repository.MyBookRepository

class DeleteMyBookUseCaseTest {
    @MockK
    private lateinit var myBookRepository: MyBookRepository
    private lateinit var useCase: DeleteMyBookUseCase

    @Before
    fun setUp() {
        MockKAnnotations.init(this)
        useCase = DeleteMyBookUseCase(myBookRepository)
    }

    @Test
    fun `invoke returns success flow from repository`() = runTest {
        // Given
        val mybookId = 123
        val expectedFlow = flowOf(DataResource.Loading(), DataResource.Success(Unit))
        coEvery { myBookRepository.deleteMyBook(mybookId) } returns expectedFlow

        // When
        val results = useCase.invoke(mybookId).toList()

        // Then
        assertEquals(2, results.size)
        assert(results[0] is DataResource.Loading)
        assert(results[1] is DataResource.Success)
        coVerify(exactly = 1) { myBookRepository.deleteMyBook(mybookId) }
    }

    @Test
    fun `invoke returns error flow from repository`() = runTest {
        // Given
        val mybookId = 123
        val errorMessage = "Failed to delete book"
        val expectedFlow = flowOf(
            DataResource.Loading<Unit>(),
            DataResource.Error(errorMessage)
        )
        coEvery { myBookRepository.deleteMyBook(mybookId) } returns expectedFlow

        // When
        val results = useCase.invoke(mybookId).toList()

        // Then
        assertEquals(2, results.size)
        assert(results[0] is DataResource.Loading)
        assert(results[1] is DataResource.Error)
        assertEquals(errorMessage, (results[1] as DataResource.Error).message)
        coVerify(exactly = 1) { myBookRepository.deleteMyBook(mybookId) }
    }

    @Test
    fun `invoke passes correct mybookId to repository`() = runTest {
        // Given
        val mybookId = 456
        val expectedFlow = flowOf(DataResource.Success(Unit))
        coEvery { myBookRepository.deleteMyBook(mybookId) } returns expectedFlow

        // When
        useCase.invoke(mybookId).toList()

        // Then
        coVerify(exactly = 1) { myBookRepository.deleteMyBook(mybookId) }
    }
}
